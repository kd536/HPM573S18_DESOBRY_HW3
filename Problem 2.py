...
class ChanceNode(Node):
    ...
    def get_health_utility(self):
        exp_util = self.utility  # expected utility initialized with the cost of visiting the current node
        i = 0
        for node in self.futureNodes:
            exp_util += self.probs[i]*node.get_health_utility()
        return exp_util
...
print(C1.get_health_utility())
print(C3.get_expected_cost())
print(C3.get_health_utility())
